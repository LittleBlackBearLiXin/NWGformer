...
from eval import *
from parseS import parse_method, parser_add_main_args
...
S = save_or_load_shortest_paths(dataset.graph['edge_index'], n,k=1,filename=args.dataset)
dataset.graph['node_feat']=torch.cat((dataset.graph['node_feat'],S),dim=1)
...
for run in range(args.runs):
    ...
    #scheduler = ReduceLROnPlateau(optimizer, min_lr=1e-5, factor=0.7, patience=50, verbose=True)#computer
    #scheduler = ReduceLROnPlateau(optimizer, min_lr=1e-5, factor=0.5, patience=50, verbose=True)#photo
    #scheduler = ReduceLROnPlateau(optimizer, min_lr=1e-9, factor=0.0001, patience=4, verbose=True)  # CS
    #scheduler = ReduceLROnPlateau(optimizer, min_lr=1e-5, factor=0.7, patience=1, verbose=True)#phy
    ...
    for epoch in range(args.epochs):
        ...
        if result[1] > best_val:
            ...
            if args.save_model:
                save_model(args, model, optimizer, run)
        best_tests_for_run.append(result[2])
