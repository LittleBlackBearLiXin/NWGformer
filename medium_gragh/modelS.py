...
class localmpnn(torch.nn.Module):
    def __init__(self,input_dim,output_dim,P=2,M=1,sys=True,use_weight=False):
        super(localmpnn, self).__init__()
...
class NWGformerX(torch.nn.Module):
    def __init__(self,input_dim,output_dim,droupout=0.2,P=2,M=1,alpha=0.8,bn=False,ln=False,res=False,use_weight=False,gnn='gat'):
        super(NWGformerX, self).__init__()
        self.linq1 = torch.nn.Linear(input_dim, output_dim)
        self.linv1 = torch.nn.Linear(input_dim, output_dim)
        if gnn == 'gat' and P != 0:
            self.conv=localmpnn(input_dim, output_dim)
        else:
    ...
    def former(self, Q, V, S):
        ...
        return (xout * self.alpha + sout * (1-self.alpha)) / (xnorm * self.alpha + snorm * (1-self.alpha) + self.eps)
...
class MPNNs(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, local_layers=3, 
                 dropout=0.5, heads=1, pre_ln=False, pre_linear=False, res=False, ln=False, bn=False, jk=False, gnn='gat'):
        ...
        if self.pre_linear:
            self.former = NWGformerX(hidden_channels, hidden_channels, droupout=self.dropout, P=2, M=1,alpha=0.3, bn=self.bn,ln=self.ln, res=self.res, gnn=gnn)
        else:
            self.former=NWGformerX(in_channels, hidden_channels,droupout=self.dropout,P=2,M=1,alpha=0.8,bn=self.bn,ln=self.ln,res=self.res,gnn=gnn)

        #self.former2 = NWGformerX(hidden_channels, hidden_channels, droupout=self.dropout, P=2, M=1, alpha=0.3,bn=self.bn, ln=self.ln,res=self.res, gnn=gnn)
        #self.former3 = NWGformerX(hidden_channels, hidden_channels, droupout=self.dropout, P=2, M=1,alpha=0.8, bn=self.bn,ln=self.ln, res=self.res, gnn=gnn)

        if not self.pre_linear:
            if gnn=='gat':
                self.local_convs.append(localmpnn(hidden_channels, hidden_channels))
            elif gnn=='sage':
    ...
    def forward(self, x, edge_index):
        ...
        #x=self.former2(x,edge_index,s)
        #x = self.former3(x, edge_index)
        x_final = 0
