...
class MPNNs(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, local_layers=3, 
        ...
        if self.pre_linear:
            self.former = NWGformerF(hidden_channels, hidden_channels, droupout=self.dropout, P=2, M=1, bn=self.bn,ln=self.ln, res=self.res, gnn=gnn)
        else:
            self.former=NWGformerF(in_channels, hidden_channels,droupout=self.dropout,P=2,M=1,bn=self.bn,ln=self.ln,res=self.res,gnn=gnn)

        #self.former2 = NWGformerF(hidden_channels, hidden_channels, droupout=self.dropout, P=2, M=1, bn=self.bn, ln=self.ln,res=self.res, gnn=gnn)
