...
class MPNNs(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, local_layers=3,
                 in_dropout=0.0, dropout=0.2, heads=1,
        ...
        #self.former2 = NWGformerF(hidden_channels, hidden_channels)
        if self.pre_ln:
...
# 计算所有run的每个epoch对应的平均best_test
average_best_tests = np.mean(all_best_tests, axis=0)

# 打印每个epoch对应的平均best_test
...
"""
All runs:
Highest Train: 87.15 ± 0.14
Highest Test: 86.52 ± 0.06
Highest Valid: 86.13 ± 0.12
  Final Train: 87.10 ± 0.17
   Final Test: 86.11 ± 0.06
"""