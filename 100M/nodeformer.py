...
class NodeFormer(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=2, num_heads=4, dropout=0.0,
                 ...
                 use_residual=True, use_act=False, use_jk=False, nb_gumbel_sample=10, rb_order=2, rb_trans='sigmoid', use_edge_loss=True, support_neighbor_sampling=True):
        ...
        self.use_jk = use_jk
        self.use_edge_loss = use_edge_loss
        self.support_neighbor_sampling = support_neighbor_sampling
    ...
    def forward(self, x, edge_index, center_node_indices=None, tau=1.0):  # 新增 center_node_indices 参数
        n = x.shape[0]
        adjs = []
        ...
        # 新增：如果提供了中心节点索引，只返回这些节点的输出
        if center_node_indices is not None and self.support_neighbor_sampling:
            x_out = x_out[center_node_indices]
